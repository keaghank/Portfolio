...
public class CodeBook {
  ...
  /**
   * Reads the data in the HuffmanTree
   * Stores the data into a HashMap
   */
  private void readCodeFromTree() {
    freqs.forEach((k, v) -> { book.put(k, ht.lookup(k)); });
  }
  ...
  /**
   * @param ch
   * @return the associated bit of the given character ch
   * @throws EncodeException if the bit does not exist
   */
  public String encodeChar(Character ch) {
    String e = book.get(ch);
    if(e == null)
